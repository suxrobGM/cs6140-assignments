...
def run() -> None:
    ...
    for data in dataset:
        ...
        for i in range(len(correllations.columns)):
            for j in range(i):
                variablePairs.add((correllations.columns[i], correllations.columns[j]))
                correllationDict[(correllations.columns[i], correllations.columns[j])] = correllations.iloc[i, j]
